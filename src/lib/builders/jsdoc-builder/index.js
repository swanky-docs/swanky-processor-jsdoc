...
// const examplesProcessor = require('./processors/examples-processor');
const Package = require('dgeni').Package;

// Create empty page object ready to collect data
let page = {};

const ReactDocsBuilder = function(item, doc) {
  page.item = item;
  page.data = doc;
};

ReactDocsBuilder.prototype.Package = new Package('react-docs-builder', [
  require('dgeni-packages/jsdoc'),
  require('dgeni-packages/nunjucks')
])
  // accepts links to external files for examples
  ...
  .config(function(renderDocsProcessor) {
    ...
    // should we show the live edit region
    if (page.item.preprocessor['swanky-processor-react'] &&
      page.item.preprocessor['swanky-processor-react'].hasOwnProperty('liveEdit')) {
      renderDocsProcessor.extraData.isLiveEdit = !!page.item.preprocessor['swanky-processor-react'].liveEdit;
    } else {
  ...
  .config(function(templateFinder) {
    ...
    if (page.item.preprocessor['swanky-processor-react'] && page.item.preprocessor['swanky-processor-react'].hasOwnProperty('templates')) {
      templateFinder.templateFolders.unshift(path.join(process.cwd(), `${page.item.preprocessor['swanky-processor-react'].templates}/`));
    } else {
...
module.exports = ReactDocsBuilder;
